<template>
  <div>
    ...
    <div class="apple-card mb-4">
      <el-input v-model="keyword" placeholder="搜索账号/姓名/电话" clearable :prefix-icon="Search" class="max-w-xs" @keyup.enter="load" @clear="load" />
    </div>
    ...
    <el-dialog v-model="batchDialog" title="批量创建技术员" width="560px">
      <div class="text-sm text-apple-gray mb-2">每行一个账号，格式：<code>账号,密码,姓名,联系方式</code>（联系方式可省略）</div>
...
const formRef = ref();
const form = reactive({ id: null, username: '', name: '', password: '', phone: '', status: 1 });
const rules = {
...
function openCreate() {
  editing.value = false;
  Object.assign(form, { id: null, username: '', name: '', password: '', phone: '', status: 1 });
  dialog.value = true;
}
function openEdit(row) {
  editing.value = true;
  Object.assign(form, { id: row.id, username: row.username, name: row.name, password: '', phone: row.phone, status: row.status });
  dialog.value = true;
}
...
async function onBatch() {
  ...
  const list2 = rows.map((l) => {
    const [username, password, name, phone] = l.split(',').map((s) => (s || '').trim());
